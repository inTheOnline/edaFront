<template>
  ...
  >
    <div class="dialog-tip" v-if="batchMode">
      本次将为 <strong>{{ purchaseIds.length }}</strong> 条请购单录入同一批采购信息。
    </div>
    ...
    <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
      ...
      <el-form-item label="采购链接" prop="link">
        <el-input v-model="form.link" placeholder="可粘贴商品链接" clearable />
      </el-form-item>
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="submitting" @click="handleSubmit">提交采购</el-button>
    </template>
  </el-dialog>
</template>
...
import { addBatchPurchaseRecordByPurchaseIds, addPurchaseRecord } from "@/api/modules/buy/officePurchase";
import type { OfficePurchaseRecord } from "@/api/interface/buy/officePurchase";
...
const purchaseId = ref<number | undefined>(undefined);
const purchaseIds = ref<number[]>([]);
const getTableList = ref<() => void>();

const formRef = ref<FormInstance>();
const form = reactive<OfficePurchaseRecord>({
  officePurchaseId: 0,
  platform: "",
  link: "",
  unitPrice: 0,
...
const rules = {
  ...
  number: [{ required: true, message: "请输入采购数量", trigger: "change" }],
  buyDate: [{ required: true, message: "请选择采购日期", trigger: "change" }]
};

const dialogTitle = computed(() => (batchMode.value ? "批量采购录入" : "采购信息录入"));
...
const open = (params: OpenParams) => {
  batchMode.value = Array.isArray(params.purchaseIds) && params.purchaseIds.length > 0;
  purchaseId.value = params.purchaseId;
    ...
    try {
      if (batchMode.value) {
        ...
        await addPurchaseRecord({
          officePurchaseId: purchaseId.value as number,
          platform: form.platform,
          link: form.link,
          unitPrice: Number(form.unitPrice || 0),
          number: Number(form.number || 0),
