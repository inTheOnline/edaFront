    ...
    >
      ...
      <el-form-item label="供应商" prop="supName">
        <el-select v-model="drawerProps.row.supId" placeholder="请选择供应商" clearable>
          <el-option v-for="item in supMapList" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <!-- 物料明细 -->
...
import { OutformDataAdd, Custs } from "@/api/interface/outgoing";
import { custDetailApi, getSupMapApi } from "@/api/modules/outgoing";
// 初始化
let options: any = ref([]);
let supMapList: any = ref([]);
onMounted(async () => {
  try {
    const { data: datas } = await custDetailApi();
    options.value = datas.map(optionMap);
    const { data: supList } = await getSupMapApi();
    supMapList.value = supList.map(supMap);
  } catch (error) {
...
const rules = reactive({
  ...
  subcNum: [{ required: true, message: "请填写外发订单号" }],
...
const addMaterial = () => {
  ...
  drawerProps.value.row.maters.push({ materId: 0, number: 1 });
};
