...
const tableColumns = reactive<ColumnProps[]>(
  props.columns.map(col => {
    // 如果父组件没有提供 sortable、filters 等，则默认处理
    return {
      ...col,
      // 默认开启排序（可保留你已有逻辑）
      sortable: col.sortable ?? "custom",
      // 默认开启筛选，如果父组件传了 filters 就用传入的，否则给空数组
      filters: col.filters ?? [],
      filterMultiple: col.filterMultiple ?? true,
      // onFilter 回调仅在本地数据筛选时使用，如果是远程请求可以不填
      onFilter: col.onFilter ?? ((value: any, row: any) => row[col.prop!] === value),
    };
  })
);
// 扁平化 columns
