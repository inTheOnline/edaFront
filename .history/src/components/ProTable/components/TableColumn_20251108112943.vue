        ...
        >
          {{
            default: (scope: RenderScope<any>) => {
              ...
              if (item.inputableSelect) {
  return (
    <el-select
      v-model={scope.row[item.prop]}
      placeholder={item.inputableSelect.placeholder || '请选择'}
      filterable
      default-first-option
      style="width: 100%;"
    >
      {item.inputableSelect.list.map(opt => (
        <el-option
          key={opt[item.inputableSelect.fieldNames.value]}
          label={opt[item.inputableSelect.fieldNames.label]}
          value={opt[item.inputableSelect.fieldNames.value]}
        />
      ))}
    </el-select>
  );
