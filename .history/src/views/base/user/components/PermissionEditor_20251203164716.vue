  ...
  >
    ...
    <template #footer>
      <div class="dialog-footer">
        <el-button @click="close">取消{{  roleDtoList}}</el-button>
...
console.log("子组件收到的全部 props →", props);
...
// 初始化角色权限
watch(
  () => [props.roleList, props.roleDtoList],
  ([newRoleList, newRoleDtoList]) => {
    if (!newRoleList.length || !newRoleDtoList.length) return;

    // 初始化权限
    newRoleList.forEach((r) => {
      const dto = newRoleDtoList.find(d => d.id == r.value);

      formMap[r.value] = dto
        ? { enabled: dto.enabled, access: dto.access || [] }
        : { enabled: true, access: [] };
    });

    // 默认选中第一个角色
    if (!activeRole.value && newRoleList.length > 0) {
      activeRole.value = String(newRoleList[0].value);
    }
