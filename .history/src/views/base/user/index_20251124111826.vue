  ...
  <div class="user-container">
    ...
    >
      <el-tabs type="border-card">
        <el-tab-pane v-for="item in dictStore.dictMap['role']" :key="item.value" :label="item.label" :value="item.value">
          <!-- 内容 -->
          <el-tab-pane v-for="item in dictStore.dictMap['role']" :key="item.value" :label="item.label">

            <el-form label-width="120px">
              <el-form-item label="启用角色">
                <el-switch v-model="formMap[item.value].enabled"></el-switch>
              </el-form-item>
            
              <el-form-item label="角色权限">
                <el-checkbox-group v-model="formMap[item.value].access">
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" :icon="CirclePlus" @click="winFlag = true">添加角色</el-button>
          <el-button @click="winFlag = false">取消</el-button>
          <el-button type="primary" @click="winFlag = false">
            确认修改
          </el-button>
        </div>
      </template>
