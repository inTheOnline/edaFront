<template>
  <el-drawer v-model="drawerVisible" :destroy-on-close="true" size="800px" :title="`${drawerProps.title}生产记录`">
    ...
    >
      <el-form-item label="序号" prop="id" v-if="drawerProps.row.id">
        <el-input v-model="drawerProps.row.id" disabled></el-input>
      </el-form-item>
      ...
      <el-form-item label="产品名称" prop="pid">
        <el-select v-model="drawerProps.row.p_id" placeholder="选择产品" :disabled="drawerProps.isView">
          <el-option
            v-for="item in drawerProps.materialList"
            :key="item.value"
            :label="`${item.value}（${item.label}）`"
      ...
      <el-form-item label="操作员" prop="operator_id">
        <el-select v-model="drawerProps.row.operator_id" placeholder="选择操作员（可选）" :disabled="drawerProps.isView">
...
interface DrawerProps {
  ...
  materialList: { value: number | string; label: string }[];
...
const rules = reactive({
  ...
  p_id: [{ required: true, message: "请选择产品", trigger: "change" }],
