...
onMounted(async () => {
  ...
  map.value=(await getMap()).data;
  mapTest.value =[{value:4,label:"hhh"}];
  // console.log(dictStore.dictMap["mater"]);
  console.log(mapTest.value);
...
const columns: ColumnProps[] = reactive([
  ...
  },
  {
    label: "产品编号",
    prop: "pid",
    // enum: computed(()=> {console.log(proTableRef.value?.tableData); return map.value}),
    fieldNames: { label: "labele", value: "value" },
    enum: mapTest.value,
    search: {
      el: "input",
      tooltip: "输入产品编号搜索",
