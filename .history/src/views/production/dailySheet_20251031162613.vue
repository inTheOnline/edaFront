    ...
    <div>
      ...
      >
        <template #tableHeader="scope">
          ...
          <el-button type="primary" :icon="Upload" plain @click="batchAdd">批量添加生产记录</el-button>
...
// 注意：请创建对应的抽屉组件
import ProductionDrawer from "@/views/production/components/ProductionDrawer.vue";
...
onMounted(async () => {
  ...
  await dictStore.loadDicts(["material", "staff"]);
});
...
const columns: ColumnProps[] = reactive([
  ...
  },
  {
    label: "产品名称",
    prop: "p_id",
    formatter: (row) => dictStore.getLabel("material", row.p_id) || "未知产品",
    search: {
      el: "select",
      enum: computed(() => dictStore.dictMap["material"]),
      tooltip: "选择产品名称搜索",
    },
    width: 150,
    align: "center",
  ...
  },
  {
    label: "姓名",
    prop: "operator_id",
    formatter: (row) => (row.operator_id ? dictStore.getLabel("staff", row.operator_id) : "无操作员"),
    search: {
      el: "select",
...
const openDrawer = async (title: string, row: Object = {}) => {
  const params = {
    ...
    isView: title === "查看",
    row: { ...row },
    api: title === "新增" ? addProduction : title === "编辑" ? editProduction : undefined,
    getTableList: proTableRef.value?.getTableList,
    // 传递字典数据给抽屉组件
    materialList: dictStore.dictMap["material"] || [],
